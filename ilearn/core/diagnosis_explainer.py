    ...
    def explain_attribution(
        ...
        *,
        ...
        if error_type in {"concept_gap", "conceptual"}:
            return (
                f"在「{skill_id}」上判为「{label}」：相关错题约 {wrong_count} 次，"
                f"错误模式一致指向同一概念理解问题——{detail}。"
            )
        if error_type in {"calc_error", "procedural"}:
            return (
                f"在「{skill_id}」上判为「{label}」：错题约 {wrong_count} 次，"
        ...
        return (
            f"在「{skill_id}」上出现「{label}」相关错误（约 {wrong_count} 次）。"
            f"建议结合错题回顾关键步骤。"
        )
    ...
    def build_explanations(
        ...
        *,
        ...
        for finding in cognitive_findings or []:
            skill = str(finding.get("gap_skill") or finding.get("skill_id") or "")
            if not skill:
                continue
        ...
        skill = weak_skills[0] if weak_skills else "相关知识点"
